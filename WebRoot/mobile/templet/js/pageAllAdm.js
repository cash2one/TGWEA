...
var iconfig = function($stateProvider, $urlRouterProvider) {
    ...
    }).state("adminWaitingAudit",{
        url:"/adminWaitingAudit",
        templateUrl:"adminWaitingAudit.html",
        controller:"adminWaitingAudit"
    });
...
/*adminWaitingAudit PageEvt{*/
var admWaitingAuditCtrl = function($scope, $http, $location, $timeout,$ionicPopup){
    $scope.noMoreHide = true;
    $scope.loadingHide = false;
    $scope.pageNum = 2;
    var _isLodeMore = true;
    ...
    $scope.ashowConfirm = function(index) {
        $scope.aiShowConfirm = $ionicPopup.confirm({
            title:"提 示",
            content:"您请确定生成该订单吗?",
            buttons:[ {
                text:"取 消",
        ...
        $scope.aiShowConfirm.then(function(res) {
            if (res) {
                $scope.orderNum = $scope.waoLists[index].waorderId;
                _index = index;
                getOrPostRequestObj.adminWaitingAuditConnect.adminGenerate($scope, $http);
            } else {}
        });
    };
    ...
    $scope.$on("$stateChangeStart", function() {
        $timeout.cancel($scope.loadHttp);
        if($scope.aiShowConfirm){
            $scope.aiShowConfirm.close();
        }
        if($scope.iShowAlert){
            $scope.iShowAlert.close();
        }
    });
    $scope.waitingRefresh = function() {
        $timeout(function() {
            getOrPostRequestObj.adminWaitingAuditConnect.adminWaitingAuditList($scope, $http);
            $scope.$broadcast("scroll.refreshComplete");
        }, 2000);
    };
    $scope.aWaitingAuditInit_CallBack = function(data) {
    ...
    $scope.aWaitingAuditLoad_CallBack = function(data) {
    ...
    $scope.generateOrderDone_CallBack = function(data){
        if(data.status === "S"){
            $scope.showAlert("提 示","订单生成成功!");
            $scope.waoLists.splice(_index,1);
        }else if(data.status === "F"){
            $scope.showAlert("提 示","订单生成失败!");
        }
    };
    ...
    $scope.waitingAuditLoadMore = function() {
        if ($scope.noMoreHide && _isLodeMore === false) {
            _isLodeMore = true;
            $scope.loadHttp = $timeout(function() {
                getOrPostRequestObj.adminWaitingAuditConnect.adminWaitingAuditLoadMore($scope, $http);
                $scope.$broadcast("scroll.infiniteScrollComplete");
            }, 2000);
        } else {
            $scope.$broadcast("scroll.infiniteScrollComplete");
...
admWaitingAuditCtrl.$inject = ["$scope", "$http", "$location", "$timeout","$ionicPopup"];
indexModule.controller("adminWaitingAudit",admWaitingAuditCtrl);
/*}*/
...
var detailedCtrl = function($scope,detaiService,$http,$timeout,$location) {
    ...
    $scope.isToAlbb = true;
    var pageNUm = $location.search().dNum; //f == pageNUm
    var proPage = $scope.getProduct(); //s == proPage
    var catePage = $scope.getCategory(); // t == catePage
    $scope.setCategory(0);
    $scope.setProduct(0);
    ...
    $scope.detailedInit_CallBack = function(data) {
        ...
        for (var i = 0, len = data.respData[0].prodExternals.length; i < len;i++) {
            if (data.respData[0].prodExternals[i].externalName === "murl") {
                mobileUrl = data.respData[0].prodExternals[i].externalValue;
            } else {
                pcUrl = data.respData[0].prodExternals[i].externalValue;
            }
        }
    ...
    $scope.detailedReturnData = function(data) {
        var idata,
            changePro = true,
            toAlbb = true,
            idatas = data.respData,
            iextFeatrues = [],
            slides = [],
            slideImg = "",
            ifeatureName = "",
            ifeatureValue = "";
        for (var i = 0, ilen = idatas[0].prodImgs.length; i < ilen;i++) {
            slideImg = privateVerifictionObj.verifiction.imgLink(idatas[0].prodImgs[i].thumbUrl);
            slides.push({
                iSlideImg:slideImg
            });
        }
        for (var j = 0, jlen = idatas[0].prodFeatures.length; j < jlen;j++) {
            ifeatureName = idatas[0].prodFeatures[j].featureName;
            ifeatureValue = idatas[0].prodFeatures[j].ifeatureValue;
            iextFeatrues.push({
        ...
        idata = {
            detailedTitle:idatas[0].prodName,
            islides:slides,
            changePro:changePro,
            toAlbb:toAlbb,
            price:"¥" + idatas[0].stdPrice,
            balance:idatas[0].dispStockBalance,
            extFeatrue:iextFeatrues,
            data:idatas[0].createDate,
            remark:idatas[0].remark,
            external:idatas[0].prodExternals
        };
        return idata;
    };
    $scope.$on("$stateChangeStart", function() {
        ...
        if (iLocation === "/waitingAudit" || iLocation === "/myOrder" || iLocation === "/superPro") {
            $scope.setPerSpCar(0);
        }
...
/*Register PageEvt{*/
var register = function($scope, $http, $ionicLoading, $location) {
    var foucsStyle = {
        color:"#ef575c"
    };
    var blurStyle = {
    ...
    $scope.registerSend = function() {
        if (privateVerifictionObj.verifiction.iphonever($scope.register.rPhone) === false) {
            ...
            if (privateVerifictionObj.verifiction.isLetterAndNum($scope.register.rPass) === false) {
                ...
                if ($scope.register.rPass === "" || $scope.register.rPass === undefined || $scope.register.rPassC === "" || d.register.rPassC === undefined) {
                    $scope.showAlert("提 示", "密码不能为空!");
                } else {
...
/*}*/
register.$inject = [ "$scope", "$http", "$ionicLoading", "$location" ], indexModule.controller("registerCtrl", register);
...
var shoppingCarCtrl = function($scope, $http, shoppingCarService, $ionicPopup, $location, $timeout, $ionicLoading) {
    ...
    $scope.sendOrder_CallBack = function() {
        ...
        $location.path("/waitingAudit").replace();
    };
    $scope.showDetailed = function(index) {
