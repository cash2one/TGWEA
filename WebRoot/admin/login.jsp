...
<body>
	<div class="login-bg">
		<div class="login-title">
			<img src='${ctx}/images/bgindex/gear.png' /><span>系统管理中心</span>
		</div>
		<div class="login-panel">
			...
			<div class="login-mod">
				...
				<form action="${ctx}/admin/wLogin.action" method="post"
					...
					<div style="float:left;margin-top:0.5rem;"><input type="text" id="checkcode" placeholder="验证码" maxlength="4" name="captchaimage" style="width:4.5rem;text-align:center;font-size:20px;font-family:'微软雅黑';background-repeat: no-repeat;background-position:2px;" /></div>
						<div style="float:left;margin-left:1rem;"><img id="captchaimage" src="${ctx}/common/captchas.action?uuid=1" onclick="this.src='${ctx}/common/captchas.action?uuid=1&r=' + Math.random(); " title="看不清楚？点击图片刷新" style="width:9.4rem;height:2.5rem;cursor:pointer;vertical-align: middle;" /></div>
		...
		$("#checkcode").keyup(function(){
			...
			if(tv.length == 4){
                ...
                var param = {
			        ...
			        j4tg.ajaxPost("${ctx}/admin/WadminRegcode.action", "json", false,param,
			        function(data) {
