...
<script type="text/javascript">
    ...
    function f_del(rowid) {
    	var id = manager.getRow(rowid).ownerNum;
        var param = {
            "id": id
        };
    ...
    function f_save(rowid) {
    	 ...
    	 var userPartner = $.getGridEditColumnData(g, rowid, 'ownerPartner');
         if (string.isInvalid(userPartner)) {
             $.ligerDialog.error('合作身份者ID 不能为空');
             return;
         }
    	 var userKey = $.getGridEditColumnData(g, rowid, 'ownerKey');
         if (string.isInvalid(userKey)) {
             $.ligerDialog.error('交易安全检验码 不能为空');
             return;
         }
         
    	 var ownerAccountName = $.getGridEditColumnData(g, rowid, 'ownerAccountName');
         if (string.isInvalid(userKey)) {
             $.ligerDialog.error('卖家账户 不能为空');
             return;
         }

        var param = {
            "alipayKeys.ownerNum": manager.getRow(rowid).ownerNum,
