...
(function ($)
{
    ...
    $.ligerui.controls.PopupEdit.ligerExtend($.ligerui.controls.Input, {
        ...
        {   
            if (!value) return;
            var g = this, p = this.options;
            //通过增加popupedit的parms参数处理对function的支持来修改grid参数无法动态更改的问题
            //使popupedit显示的grid能够根据主页面的参数变化而变化 edit by liuqing
            var gridOptions;
            if (typeof (p.parms) == "function")
            {
            	gridOptions = $.extend(p.grid,{
            		parms: p.parms()
            	});
            }else{
            	gridOptions = $.extend({
        ...
        function show()
        {
            ...
            if (p.condition)
            {
                ...
                $("div", containerBtn1).ligerButton({
                    ...
                    click: function ()
                    {
                		//popupedit根据动态表单数据搜索加载服务器数据grid时，整合动态表单的数据与默认的grid请求参数进行结果数据刷新 edit by liuqing
                        if(p.grid.dataAction='server'){
                        	var rules = $.ligerui.getServerConditions(conditionPanel);
                        	rules.push({ name: grid.options.pageParmName, value: grid.options.newPage }); 
                        	rules.push({ name: grid.options.pagesizeParmName, value: grid.options.pageSize }); 
                        	for(var key in grid.options.parms){
                        		var paramvalue = grid.options.parms[key];
                        		//整合动态表单的数据与默认的grid请求参数(可能存在外部传入的默认参数) edit by liuqing
                        		if(paramvalue&&!rules[key]){
                        			rules.push({ name: key, value: paramvalue });
                        		}
                			}
                        	grid.loadServerData(rules); 
                	 	}else{
            ...
            grid.refreshSize();
//            alert($(panle).find("[id]^=Form").length);
        }
        //增加获取多层JSON内容的方法，对含.的字段进行多层遍历  edit by liuqing
        function getDeepLayerData(data,fieldName){
        	var fieldLayers = fieldName.split(".");
        	for (var i = 0; i < fieldLayers.length; i++) { 
        		data = data[fieldLayers[i]];
        	}
        	return data;
        }
        ...
        function toSelect()
        {
            ...
            {//获取带.的复杂field值  edit by liuqing
                p.valueField && value.push(getDeepLayerData(rowdata,p.valueField));
                p.textField && text.push(getDeepLayerData(rowdata,p.textField));
            ...
            {//获取带.的复杂field值  edit by liuqing
                p.valueField && removeValue.push(getDeepLayerData(rowdata,p.valueField));
                p.textField && removeText.push(getDeepLayerData(rowdata,p.textField));
                var o = $.extend(true, {}, this);
                grid.formatRecord(o, true);
                removeData.push(o);
            });
