    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        setTree: function (tree)
        {
            ...
            else
            {
                ...
                {	
                	//修改只能选择叶子节点的COMBOBOX重复点击会取消选择的情况。edit by liuqing
                	if (p.treeLeafOnly) return;
                    g._changeValue("", "");
                };
            }
                ...
                $("div", containerBtn1).ligerButton({
                    ...
                    click: function () { 
	                    ...
	                    if(p.grid.dataAction='server'){
	                    	...
	                    	rules.push({ name: g.grid.options.pageParmName, value: g.grid.options.newPage }); 
	                    	rules.push({ name: g.grid.options.pagesizeParmName, value: g.grid.options.pageSize }); 
	                    	for(var key in g.grid.options.parms){
	                    		var paramvalue = g.grid.options.parms[key];
	                    		//整合动态表单的数据与默认的grid请求参数(可能存在外部传入的默认参数) edit by liuqing
	                    		if(paramvalue&&!rules[key]){
	                    			rules.push({ name: key, value: paramvalue });
	                    		}
	            			}
	                    	g.grid.loadServerData(rules); 
	            	 	}else{
