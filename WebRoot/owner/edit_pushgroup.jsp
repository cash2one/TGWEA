			...
			成员客户：
	                ...
	                <i class="i-spacing-follow">
	                        <ul>
                                ...
                                <li >
                                    <i>
					                    <div id="custlist" style="width:600px;clear:both;">
				                    	</div>
                                    </i>
                                </li>
                            </ul>
...
<script type="text/javascript">
    ...
    function f_initEditPushGroupData() {
        ...
        j4tg.ajaxPost("${ctx}/owner/wGetEditPushGroupInitData.action", "json", false, {"groupNum":groupNum},
        function(data) {
            if (data.status == "S") {
                //             				alert(JSON.stringify(data));
                if (data.respData.pushGroup) {
                    var pushGroup = data.respData.pushGroup;
                    var pushGroupCustRefs = data.respData.pushGroupCustRefs;
                    $("#groupName").val(pushGroup.groupName);
                    $("#remark").val(pushGroup.remark);
                    var custs = [];
                    for(var key in pushGroupCustRefs){
                    	custs.push(pushGroupCustRefs[key].cust);
                    }
    ...
    function f_savePushGroup() {
        ...
        var param = {
        	"wpushGroup.pushGroup.groupNum": groupNum,
            "wpushGroup.pushGroup.groupName": groupName,
            "wpushGroup.pushGroup.remark": remark,
            "wpushGroup.custNums": groupcusts

        };

        j4tg.ajaxPost("${ctx}/owner/wSaveEditPushGroup.action", "json", false, param,
        function(data) {

            if (data.status == "S") {
                $.ligerDialog.success('保存成功');
                location.href = "${ctx}/owner/wListPushGroups.action";
            }
