       ...
       <div position="top">
        ...
        <div id="maincontent" position="center" title=" ">
             <div id="beautiful">
               ...
               <div id="whereimg"><img src="../images/beautiful-page/list_pushgroups.webp"/></div>
               <div id="whatfont">推送组管理</div>
                <div class="both"></div>
               </div>
			<div class="bcolor borderradius adminico" style="margin-left:33rem;"><a name="addStkAllocInv" href="${ctx}/owner/wEditPushGroup.action"><img src='${ctx}/images/ico/addtzhu.png' alt='添加推送组' title='添加推送组'/></a></div>
            <br/>
            <div id="maingrid">
            </div>
        </div>
...
<script type="text/javascript">
        ...
        g = manager = $("#maingrid").ligerGrid({
            ...
            },
            {
                ...
                render: function(rowdata, rowindex, value) {
                    var h = "";
                    h += "<a href='${ctx}/owner/wEditPushGroup.action?groupNum=" + rowdata.groupNum + "'><img src='${ctx}/images/ico/modification.png'/></a> ";
            ...
            url: "${ctx}/owner/wListPushGroupsData.action",
            method: "post",
            usePager: false,
            async: false,
            enabledSort: true,
            ...
            detail: {
                onShowDetail: f_showPushGroupCustData,
                height: 'auto'
            }
    ...
    function f_deletePushGroup(rowid) {
        ...
        function(yes) {
            if (yes) {
                ...
                j4tg.ajaxPost("${ctx}/owner/wDelPushGroup.action", "json", false, {
    ...
    function f_showPushGroupCustData(row, detailPanel, callback) {
        ...
        j4tg.ajaxPost("${ctx}/owner/wGetPushGroupCusts.action", "json", false, {"groupNum":groupNum},
                function(data) {
                    if (data.status == "S") {
                        //             				alert(JSON.stringify(data));
                        if (data.respData) {
                            var custs = data.respData;
                        	for (var key in custs) {
                                var cust = custs[key];
                                var custobj = [];
    ...
    function f_delPushGroup(rowId) {
    	var pushgroup = manager.getRow(rowid);
        $.ligerDialog.confirm('确定要删除么?',
        function(yes) {
            if (yes) {

                j4tg.ajaxPost("${ctx}/owner/wDelPushGroup.action", "json", false, {
                    "groupNum": pushgroup.groupNum
                },
                function(data) {
                    if (data.status == "S") {
                        $.ligerDialog.success('删除成功');
