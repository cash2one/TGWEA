...
(function ($)
{
    ...
    //获取请求服务器的动态生成的form表单参数,popupedit调用时使用  edit by liuqing
    $.ligerui.getServerConditions = function (form, options)
    {
        if (!form) return null;
        form = liger.get($(form));
        if (form && form.toServerConditions) return form.toServerConditions();
    };
    ...
    $.ligerDefaults.Form.editorBulider = function (jinput)
    {
        ...
        options = $.extend({
        	//屏蔽Form表单里编辑框的长度不变无法更改的问题 edit by liuqing
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        toServerConditions: function ()
        {
            ...
            $(p.fields).each(function (fieldIndex, field)
            {
                ...
                if (value)
                {
                	conditions.push({ "name": name, "value": value });
                }
            });
            return conditions;
        },
        ...
        _setFields: function (fields)
        {
            ...
            (function ()
            {
                ...
                $(fields).each(function (fieldIndex, field)
                {
                    ...
                    //修改popupedit调用导致form表单没有放置在body内，获取不到container导致无法现在搜索框元素的BUG edit by liuqing
                    if (!container){ 
                    	container = $("#"+g.id + "\\|" + fieldIndex,jform);
                    	if(!container.length){
                    		return;
                    	}
                    }
        ...
        _buliderControlContainer: function (field, fieldIndex)
        {
            ...
            if (p.fieldCss)
            {	
            	out.push(' class="' + (g.id + "|" + fieldIndex) + '"');
            }
