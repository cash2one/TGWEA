...
public class WOwnerAction extends WCommonBaseAction{
	...
	@Action(value="wUpdateGenerateProject")
	public String updateGenerateProject() throws IOException{
		...
		generateProject.setUpdateBy(getCurrentAdmin().getAdminName());
		iwGenerateProjectService.updateWGenerateProject(generateProject);
	...
	@WeaFunction(fname="恢复企业服务",oname=WeaFunction.Operation.UPDATE)
	@Action(value="wStartOwner")
	public String startOwner() throws IOException {
		iwOwnerService.updateStartSuspendOwner(ownerNums);
		return JSONRESPONSE;
	}
